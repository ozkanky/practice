...
module.exports = {
  ...
  update: async (req, res) => {
    ...
    const data = await Token.updateOne({ _id: req.params.TokenId }, req.body, {
      runValidators: true,
    }); //,Token modelinde  {_id: req.params.TokenId }kaydını  updateOne yap, -> req.body kaydıyla update yap,-> { runValidators: true }eger bir update yapılmışsa validasyonlar uygulanır
    res.status(202).send({
      error: false,
      data,
